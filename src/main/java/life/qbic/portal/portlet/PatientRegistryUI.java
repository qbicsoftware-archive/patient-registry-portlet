...
/**
 * Entry point for portlet patient-registry-portlet. This class derives from {@link QBiCPortletUI}, which is found in the {@code portal-utils-lib} library.
 */
@Theme("mytheme")
@SuppressWarnings("serial")
@Widgetset("life.qbic.portlet.AppWidgetSet")
public class PatientRegistryUI extends QBiCPortletUI {

    private static final Logger LOG = LogManager.getLogger(PatientRegistryUI.class);
    ...
    private static final boolean USING_OPEN_BIS = true;
    private static final boolean USING_QBIC_DATABASES = true;

    private Layout REMOVE_THIS_METHOD_AND_DO_YOUR_OWN_THING_COMMA_PLEASE(final VaadinRequest request) {
        LOG.info("Generating content for portlet patient-registry-portlet");
        ...
        builder.append("<br/>You can now start developing. Start by modifying the <font face='monospace'>getPortletContent</font> method in the generated <font face='monospace'>src/main/java/life/qbic/portal/portlet/PatientRegistryUI.java</font> file.<br/><br/>");
        ...
        final Panel mainPanel = new Panel("Patient Registry");
    ...
    private void insertOpenBISConnectionTesterComponents(final Layout layout) {
        ...
        openBisTestButton.addClickListener((Button.ClickListener) event -> {
            if (isOpenBISWorking(dataSourceUser.getValue(), dataSourcePassword.getValue(), dataSourceUrl.getValue())) {
                Styles.notification("Patient Registry", "Connection to openBIS works!", NotificationType.SUCCESS); 
            } else {
                Styles.notification("Patient Registry", "Could not establish openBIS connection. Check the logs for exceptions.", NotificationType.ERROR); 
            }
        });
    ...
    private void insertInternalDatabasesConnectionTesterComponents(final Layout layout) {
        ...
        internalDatabaseTestButton.addClickListener((Button.ClickListener) event -> {
            if (isQBiCDatabaseWorking(username.getValue(), password.getValue(), jdbcUrl.getValue())) {
                Styles.notification("Patient Registry", "Connection to QBiC database works!", NotificationType.SUCCESS); 
            } else {
                Styles.notification("Patient Registry", "Could not establish connection to QBiC database. Check the logs for exceptions.", NotificationType.ERROR); 
            }
        });
